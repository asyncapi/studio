...
export class SplitView extends EventEmitter implements Disposable {
  ...
  constructor(
    ...
    if (options.descriptor) {
      ...
      for (const [
        index,
        viewDescriptor,
      ] of (options.descriptor.views as any).entries()) { // it's a iterator, any type is for fixing TS
  ...
  // 	this.addView(view.element, view, sizing, to);
  // }
  
  // public swapViews(from: number, to: number): void {
  // 	if (from > to) {
  ...
  // 	this.addView(toView.element, toView, fromSize, from);
  //   this.addView(fromView.element, fromView, toSize, to);
  // }
